...
% % x1 = [0.87,1.35; 3.005,3.405; 4.75,0.623; -1.66,2.877; -3.28,0.69;
% %     -1.326,-1.014; -4.379,-1.757; -0.889,-3.352; 1.393,-1.249;
% %     2.566,-4.121];
% % x2 = [7.307,2.690; 6.0100,7.233; -0.65,6.955; -5.158,8.015;
% %     -6.894,2.570; -2.1135,-6.92; -7.932,-3.437; 2.650,-8.6145;
% %     4.2460,-5.702; 8.298,-3.282];
% % 
% % X1 = x1.^2;
% % X2 = x2.^2;
% % X = [X1;X2];Y = [ones(10,1);-ones(10,1)];
% % %%%% p1 = 12  q1 = -10



% load CosExp_Original.mat X Y
% load CosExp_Noisy.mat X Y

%     load syn_ori_214.mat % ori
    load syn_214_010.mat % noise
...
%■■■ 降 维 ■■■
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function  X_redc = PCA(X,Y,FunPara)
% [eigvector, ~] = eigs(cov(X),FunPara.dim1); % return the diml largest magnitude eigenvalues.
% X_redc=X*eigvector;
% end

% % Propotion = 0.95;   %降维选取的主成分 对 原矩阵X的贡献度截取阈值∈(0,1]
% % % Propotion = 0.99;
% % % Propotion = 0.999999999998;
% % [COEFF, SCORE, LATENT] = pca(X);    % X_n×p
% % % COEFF_p×p:每列coeff包含一个主成分的系数，而列则按成分方差的降序排列
% % % LATENT_p×1:主成分方差矩阵 即 X的协方差矩阵的特征值
% % sum_LATENT = cumsum(LATENT/sum(LATENT));    %按序累加的主成分贡献度阵_p×1
% % Dim = find(sum_LATENT >= Propotion);
% % Dim = Dim(1);   %降维后的维数
% % 
% % X = X * COEFF(:,1:Dim);   %降维_n×p
end     % PCA降维■
...
FunPara.p2 = 0;
FunPara.pnum = 1;
% FunPara.pnum = 2;   % nu-SVC
% Setting，非线性需要设置参数FunPara.kerfPara.pars
%■■■■■■■■■■■■■■■■■■
FunPara.kerfPara.type = 'lin';    % 线性核
% FunPara.kerfPara.type = 'rbf';    % 高斯核
% FunPara.kerfPara.type = 'cro';    % CRO Hash using DCT
%■■■■■■■■■■■■■■■■■■
FunPara.kerfPara.pars1 = 0; % U
FunPara.kerfPara.pars2 = 0; % tao
% FunPara.kerfPara.pnum = 2;
fid = 0;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%■■■十折交叉验证■■■
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
FunPara = KfoldParaSearch_C(Data,SVMFun,FunPara,fid);
% FunPara = KfoldGmeanParaSearch(Data,SVMFun,FunPara);      %Gmean

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%■■■非十折交叉验证， 即有训练集也有预%%%%%%%%%%%%%%%%%%%%%%%
% FunPara = LIBSVMNoKfoldParaSearch(Data,SVMFun,FunPara);   %NoKfold
% FunPara = LIBSVMParaSearch(Data,SV测集■■■
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%MFun,FunPara);
