function [alpha, b] = RSVC( X_train, Y_train, K, lambda, delta)
...
X_train=X;Y_train=Y;K=X*X';lambda=1;delta=1;
% ====================== Initilize  =======================================
n = size(X_train, 1);
...
[alpha_0_coefficient, alpha_0_offset] = wwL2_SVM(K, Y_train, lambda, weight_vector);
alpha = [alpha_0_offset; alpha_0_coefficient];
...
while (norm(alpha_0 - alpha) > 1e-4 && iter_number < max_iter);
    ...
    alpha_0 = alpha;
    offset = alpha_0(1);
    coefficient = alpha_0(2:end);
    
    % updating the weight obtained by using the classification loss in Example 2:
%     residual = exp(- max(0,(1 - Y.*(K*coefficient + offset*ones(n,1)))).^2/delta^2);
    
    % updating the weight obtained by using the classification loss in Example 1:
    K*coefficient + offset*ones(n,1);
    residual = delta^2./(delta^2 + max(0, (1 - Y.*(K*coefficient + offset*ones(n,1))) ).^2);
    [coefficient, offset] = wwL2_SVM(K, Y_train, lambda, residual);
    
    alpha = [offset; coefficient];
...
alphab = [alpha_0_coefficient;alpha_0_offset];      e = ones(n, 1);    sig2=1;  lam=1;
% wxb = K * alpha + b*e;
% PredictY = sign( wxb );
% AC = sum(PredictY==Y)/n


function [output_alpha, offset] = wwL2_SVM(K, Y, lambda, w)
